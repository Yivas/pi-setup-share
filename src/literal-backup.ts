...
import { materializeTreeArchive, previewTreeArchive, type LiteralPreview, type LiteralPreviewOptions } from './literal-archive.ts';
...
// Second pass for restore: materialize the verified backup into a directory that must not exist yet.
export async function materializeReceiverBackup(path: string, destination: string, options: LiteralPreviewOptions = {}): Promise<LiteralPreview> {
  return materializeTreeArchive(path, destination, BACKUP_ARCHIVE_SPEC, options);
}
